...
#include "parsers/instructions.hpp"
...
int main()
{
    ...
    // Test for 'mov' parser
    // runTest(mov,
    //         {"mov r1, r2", "mov r1, &[!label]", "mov $0x1234, r1"}, // Valid inputs
    //         {"mov r1,, r2", "mov r1, $0x123", "mov r1 r2"},         // Invalid inputs
    //         "mov");

    auto mov_result = mov("mov r1, r2");
    std::cout << "mov_result.success: " << mov_result.success << std::endl;
    std::cout << "mov_result.start: " << mov_result.start << std::endl;
    std::cout << "mov_result.end: " << mov_result.end << std::endl;
    std::cout << "mov_result.parsed: ";
    for (const auto &parsed : mov_result.parsed)
    {
        std::cout << parsed << ", ";
    }
    std::cout << std::endl;
    std::cout << "mov_result.remaining: " << mov_result.remaining << std::endl;
